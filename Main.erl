...
main(_) ->
    ...
    InitLen           = length(InitS),
    Width             = InitLen + 2*Gens,
    RuleBits          = rule_bits(Rule),

    InitCells         = [if C == $1 -> 1; true -> 0 end || C <- InitS],
    ...
    ensure_results_dir(),
    File = filename(Rule, Gens, InitS),
    write_pbm(File, Width, Gens, Rows),
    io:format("Wrote ~s~n", [File]),
    halt(0).
...
%% safe zip3: stop when any list empties
zip3([], _, _)              -> [];
zip3(_, [], _)              -> [];
zip3(_, _, [])              -> [];
zip3([A|As], [B|Bs], [C|Cs]) ->
...
ensure_results_dir() ->
    case filelib:is_dir("results") of
        true  -> ok;
        false -> file:make_dir("results")
    end.
